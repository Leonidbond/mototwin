...
Расходы остаются отдельной страницей `/expenses` / `/vehicles/[id]/expenses`, но дерево узлов и Node Context показывают компактный финансовый контекст по выбранному узлу.
...
- web-дерево может показывать compact-строку `Расходы за сезон` и раскрытый блок `Расходы по узлу` для parent-узлов;
- Node Context на web и Expo под заголовком показывает две карточки: состояние обслуживания (~65%) и `Расходы по узлу` (~35%);
- `Расходы по узлу` показывает сумму за выбранный сезон и количество записей; разные валюты выводятся раздельно;
- клик по карточке ведёт на страницу расходов с `nodeId`, `year` и `returnNodeId`;
- `/expenses` / `/vehicles/[id]/expenses` трактует `nodeId` как выбранный узел плюс всё поддерево, чтобы parent node открывал ту же финансовую область, что и дерево;
- возврат со страницы расходов по `returnNodeId` возвращает в дерево узлов к исходному узлу.
...
## TOP-15 -> Overview groups mapping

На overview-странице мотоцикла и в профиле пользователя используются **динамические** карточки/группы: показываются только категории, в которых есть хотя бы один узел в текущем ТОП-наборе.

Источник списка узлов:

- API `GET /api/nodes/top` — для текущего пользователя: `UserSettings.favoriteNodeCodes` или дефолтные 15 кодов из [`src/lib/top-service-nodes.ts`](../src/lib/top-service-nodes.ts).
- Группировка: `buildTopNodeOverviewCards` / `buildTopNodeProfileGroups` в [`packages/domain/src/top-node-overview.ts`](../packages/domain/src/top-node-overview.ts).

Персонализация и UI профиля — [custom-top-nodes-mvp.md](./custom-top-nodes-mvp.md).

Фиксированные категории (узлы вне них попадают в **Прочее** / `other`):
...
- `suspension` / Подвеска:
  ...
  - `SUSPENSION.FRONT.OIL`
- `other` / Прочее:
  - любые коды из текущего ТОП-набора, не входящие в группы выше
