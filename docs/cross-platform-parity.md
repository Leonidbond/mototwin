...
- **Expo:** `apps/app/app/vehicles/[id]/wishlist/index.tsx` — те же условия (**`ORDERED` / `BOUGHT` / `INSTALLED`** + **`nodeId`**), модалка, **`POST .../wishlist`** с **`NEEDED`**, комментарий **«Повтор из корзины замен»**, без копирования стоимости; после успеха — фильтр **NEEDED** и **`applyWishlistRowFocus`** на новую строку.
- **Parity status:** aligned по смыслу и API; отличия только в chrome (web: inline overlay + панель справа; Expo: нативные `Modal` + нижний sheet деталей).
