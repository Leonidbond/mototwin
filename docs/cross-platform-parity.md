...
- **Shared:** `@mototwin/domain` (`buildPickerSubmitPreview`, `arePickerQuantityResolutionsComplete`), `@mototwin/types` (`quantityUpgrade` в `PickerSubmitDecision`, `PickerQuantitySubmitResolution` с режимами **`addAllFromDraft` / `setQtyToDraft`**), `@mototwin/api-client` (`submitPickerDraft` с `quantityResolutions`, **`updatedWishlistItemIds`**).
- **Web:** `PartPickerPage` + `PickerSubmitPreviewModal` — выбор режима для строк `quantityUpgrade`, затем **`POST`** новых SKU и **`PATCH`** существующих строк; `?picked=` на корзине из **созданных + обновлённых** id.
