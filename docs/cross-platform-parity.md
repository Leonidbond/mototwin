...
- **Local settings parity:** both clients expose local settings on dedicated Profile surface (currency, distance unit, date format, default snooze days; engine-hours unit fixed to `h`) with shared defaults/normalization in domain. Persistence is local-only (web `localStorage`, Expo local storage helper), no backend sync yet. `defaultCurrency` is wired to wishlist create + add-service-event defaults while preserving item-specific wishlist-install prefill. See [user-settings-mvp.md](./user-settings-mvp.md).
