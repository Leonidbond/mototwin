...
- **Shared:** `@mototwin/domain` (`buildPickerSubmitPreview`, `arePickerQuantityResolutionsComplete`), `@mototwin/types` (`quantityUpgrade` в `PickerSubmitDecision`, `PickerQuantitySubmitResolution`), `@mototwin/api-client` (`submitPickerDraft` с `quantityResolutions`, поле **`updatedWishlistItemIds`** в результате).
