...
Журнал — отдельная страница `/vehicles/[id]/service-log` (не модалка на карточке ТС). Добавление/редактирование сервисного события — модальное окно на этой странице.
...
- Web: отдельная страница журнала, модалка только для формы события, липкие заголовки месяцев, чипы сортировки с индикатором направления.
- Expo: полноэкранный список, сворачиваемый блок фильтров, компактные даты (`dateStyle: "compact"` в `buildServiceLogTimelineProps`); создание/редактирование/повтор — экран `/vehicles/[id]/service-events/new` с query `source=service-log` и при необходимости `eventId` / `repeatFrom`; после сохранения возврат в журнал может использовать `feedback=created|updated`.
