...
- **`year`** — сезон (синхронизируется с выбором года на странице);
- **`serviceEventId`** — в таблице «Все расходы» остаются только расходы, привязанные к этому сервисному событию;
- **`highlightExpenseId`** — после загрузки списка выполняется **скролл** к строке с `data-expense-id` и кратковременное **визуальное выделение** строки;
- **`returnTo`** — декодированный внутренний путь (префиксы **`/vehicles/`** или **`/garage`**): кнопка **«Назад»** сначала делает `router.push(returnTo)`, затем (если `returnTo` нет) срабатывает прежняя логика **`returnNodeId`** → дерево узлов, затем **`backHref`** / `history.back()` (см. `src/app/expenses/ExpensesPageClient.tsx`).

Параметры **`nodeId`** и **`returnNodeId`** для сценария «из дерева узлов» на общую страницу расходов сохраняются без изменений.
