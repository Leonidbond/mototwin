...
- **`MT-SEC-043`** (P2) — нет валидации обязательных ENV на boot:
  - `AUTH_SECRET`, `DATABASE_URL`, `SMTP_HOST`/`SMTP_USER`/`SMTP_PASS`, `AUTH_EMAIL_FROM`, `GOOGLE_OAUTH_CLIENT_ID`, `APPLE_CLIENT_ID`, `YANDEX_GEOCODER_API_KEY` — все проверяются «лениво» при первом обращении. В случае неконфигурированного боевого VPS пользователь увидит 500 при попытке OAuth или forgot-password, а не при старте Next.
...
- Reg.ru SMTP (`sendPasswordResetEmail`, `sendNotificationEmail`) — см. [email-smtp.md](../email-smtp.md) и [send-email.ts](../../src/lib/email/send-email.ts).
