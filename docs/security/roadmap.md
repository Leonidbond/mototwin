...
- `MT-SEC-068`/`069`/`070`/`071` — закрыты ~20-25 высокоприоритетных handler-ов; оставшиеся 14 admin/notification routes имеют strict zod, но без `parseJsonBody`. План: следующая итерация заведёт ESLint guard `no-restricted-syntax` на `z.object(` без `.strict()` и `await request.json()` без `parseJsonBody`, после чего покроет оставшиеся routes mechanically.
...
- `MT-SEC-054` (auth audit-log) — требует новой Prisma-модели + миграции.
...
- `MT-SEC-055` — ретеншн логов и алертинг.
...
| ID | Название | Severity (predict) | Эскиз |
|----|----------|---------------------|-------|
| `MT-SEC-029` | Nginx без TLS / HSTS / security headers | P0 (для прода) | Добавить `listen 443 ssl;`, certbot, security headers (см. MT-SEC-006). |
| `MT-SEC-027` | `mototwin.dump` в репо | P0 (если реальный) | Удалить из git history, регенерировать `AUTH_SECRET`/DB-пароль, заменить на seed без PII. |
| `MT-SEC-026` | README с SSH-IP + demo credentials | P1 | Перенести deployment-инструкции в `docs/deploy/` (gitignored или private), ротировать demo-пароль. |
