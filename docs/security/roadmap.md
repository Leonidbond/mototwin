...
## Итерация 3 — Infra hardening (частично закрыто)
...
| **scope:infra P0** | `MT-SEC-027` (partial) — `mototwin.dump` untracked из git index (`git rm --cached`) + остаётся в `.gitignore`. **Open follow-up:** rewrite git history (commit `85860f1` всё ещё содержит файл) + полная ротация секретов (`AUTH_SECRET`, `DATABASE_URL`, `RESEND_API_KEY`, `YANDEX_GEOCODER_API_KEY`); все active web-сессии и mobile refresh-токены будут инвалидированы после смены `AUTH_SECRET`. Чек-лист — [docs/deploy/vps.md §11](../deploy/vps.md#11-инцидент-mototwindump-в-git-history-mt-sec-027). |
