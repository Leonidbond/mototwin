...
Дополнение по семантике summary parity:
- для root/top-level узлов `node-tree` нормализует итоговый `effectiveStatus`:
  - если расчет дал `null`, в summary-срезе возвращается `OK`;
- для child/non-root узлов `null` сохраняется как допустимое состояние
  (статус неизвестен/невычислим).
