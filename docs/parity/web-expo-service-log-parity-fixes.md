...
- **`paidOnly`:** поле `ServiceEventsFilters.paidOnly === true` оставляет события с явной стоимостью на `ServiceEvent` **или** с привязанными строками `ExpenseItem` (`isPaidServiceEvent` / `resolveServiceEventCost` в `@mototwin/domain`). Сочетается с остальными полями фильтра и с **фильтром по узлу** (сначала подмножество по узлам, затем остальное).
...
- **Место установки (web-only map, 2026-06):** на web — picker с встроенной картой: поиск + **собственные маркеры/список** (Geosuggest через `/api/service-places/search` и `/api/geocode?nearbyBiz=1`); встроенные POI Яндекса не используются для выбора. Контракт формы и API общие (`servicePlaceId`, `servicePlaceSnapshot`, `installLocation*`). На Expo — text search + manual + deep link, **интерактивная карта отложена** (WebView — отдельная фаза) — [web-service-event-form.md](../web-service-event-form.md) §5.1.
