...
- **Parity status:** aligned as personal dashboard semantics (`Мой гараж`) with shared summary meaning, web-aligned garage cards, and platform-appropriate navigation chrome.
- **Notes:** both clients use shared `buildGarageDashboardSummary` (motorcycles count + attention counts from existing garage payload, no extra backend calls). Both clients use the same Garage card information architecture: title + meta line, silhouette, dedicated `Garage Score`, short `Требует внимания` block, and quick actions. **Quick actions (2026-06):** web card `Добавить ТО` → `service-events/new`, `Расход` → `/expenses`; Expo card `Расход` → `/expenses` (не `service-log`). **Garage CTA:** web header and dashed add-card — `<Link href="/onboarding">` без вложенного `<Button>`. Expo bottom nav **«Узлы»** с гаража/расходов/подбора → `/vehicles/{id}/nodes`. Empty state and primary CTA (`Добавить мотоцикл`) are aligned. Garage Score legend uses Russian status labels on both clients. Web keeps profile/settings entry in desktop chrome; Expo exposes it in bottom navigation. See [garage-dashboard-mvp.md](../garage-dashboard-mvp.md) and [auth-roadmap.md](../auth-roadmap.md).
- **Profile/settings parity:** both clients expose the same Profile settings surface (currency, distance unit, date format, default snooze days, **вид узлов по умолчанию**, **Мой ТОП узлов**; engine-hours unit fixed to `h`) with shared defaults/normalization in domain. Persistence is DB-backed per user via `/api/user-settings`; local storage/file cache is fallback only and scoped by user identity. `defaultCurrency` is wired to wishlist create + add-service-event defaults while preserving item-specific wishlist-install prefill. TOP customization: grouped preview (incl. **Прочее**), replace/add/remove, max 15 nodes — [custom-top-nodes-mvp.md](../custom-top-nodes-mvp.md), [user-settings-mvp.md](../user-settings-mvp.md).
...
- **Overview top-nodes:** both clients show compact overview cards powered by user-aware `GET /api/nodes/top` (default or custom 15 nodes). Only **non-empty** category blocks are rendered (Смазка, Двигатель, …, **Прочее**). Full tree remains a separate flow (`Все узлы →`). Initial **ТОП-узлы** tree filter follows `defaultNodeView` from user settings. Cards are informational surfaces, not click targets. The group icon drills into `SOON` / `OVERDUE` nodes in the group; leaf badges open the tree on the exact node. See [custom-top-nodes-mvp.md](../custom-top-nodes-mvp.md).
...
- **Web:** общая форма **`ServiceEventForm`** (`src/app/vehicles/[id]/_components/service-event-form/`) на маршрутах **`service-events/new`** и **`service-events/[eventId]/edit`** — входы со страницы журнала, карточки ТС и др.; одна модель **`AddServiceEventFormValues`** (BASIC/ADVANCED, несколько узлов, запчасти/работа, SKU, блок **«Готово к установке»** из **`getInstallableForServiceEvent`**, JSON).
...
- **ServicePlace map (2026-06):** web picker — встроенная карта, выбор организации только через серверные кандидаты (маркеры + список), не через POI-тайлы Яндекса. Expo — text search + manual + внешний deep link; интерактивная карта в приложении отложена.

## 3.6 Update vehicle state

- **Web:** inline edit in vehicle page; deep link `?openVehicleState=1` открывает модалку пробега/моточасов. Legacy `/vehicles/:id/state?focus=mileage` редиректится на дашборд с `openVehicleState=1` (`src/app/vehicles/[id]/state/page.tsx`).
- **Expo:** dedicated route (`vehicles/[id]/state`); query `?openVehicleState=1` на дашборде редиректит на `state`.
- **Parity status:** aligned
- **Notes:** both call same backend endpoint and produce `STATE_UPDATE` log side effect. Уведомление «Обновите пробег» использует `actionUrl` `/vehicles/:id?openVehicleState=1&focus=mileage`.
...
- **Kit-origin mark:** на web и Expo строки из kits помечаются одинаково: первая строка `comment` — **`Из комплекта: [KIT_CODE] Название`** (legacy: без `[CODE]`). В корзине — **плашка с кодом комплекта** рядом с названием позиции; на picker — **код (`kit.code`) перед названием** комплекта в списке и в draft-корзине. Разбор: `parseWishlistKitOriginDetails` / поля view model (без новых полей Prisma/API).
...
## 3.8.2a Parts wishlist — «Повторить покупку» (корзина замен)

- **Web:** `src/app/vehicles/[id]/parts/_components/PartsCartPage.tsx` — модалка подтверждения, пункт в меню строки и действие в правой панели деталей; см. [parts-wishlist-mvp.md(../parts-wishlist-mvp.md) § Web.
- **Expo:** `apps/app/app/vehicles/[id]/wishlist/index.tsx` — те же условия (**`ORDERED` / `BOUGHT` / `INSTALLED`** + **`nodeId`**), модалка, **`POST .../wishlist`** с **`NEEDED`**, комментарий **«Повтор из корзины замен»**, без копирования стоимости; после успеха — фильтр **NEEDED** и **`applyWishlistRowFocus`** на новую строку. Модалка подтверждения не открывается поверх уже открытого листа детали: **`openRepeatPurchaseConfirm`** закрывает другие **`Modal`**, затем планирует показ через **`InteractionManager.runAfterInteractions`**.
- **Parity status:** aligned по смыслу и API; отличия только в chrome (web: inline overlay + панель справа; Expo: нативные `Modal` + нижний sheet деталей; на RN — явная последовательность модалок для «Повторить покупку»).

## 3.8.2b Picker draft → wishlist (частичное совпадение по количеству)

- **Shared:** `@mototwin/domain` (`buildPickerSubmitPreview`, `arePickerQuantityResolutionsComplete`, **`computePickerSubmitWishlistPieceDelta`**, **`computePickerSubmitPriceEstimate`**), `@mototwin/types` (`quantityUpgrade` при совпадении **узел + SKU** в активном списке — **всегда**, независимо от соотношения количеств; `PickerQuantitySubmitResolution`: **`addAllFromDraft` / `setQtyToDraft`**), `@mototwin/api-client` (`submitPickerDraft` с `quantityResolutions`, **`updatedWishlistItemIds`**, опционально **`noOpQuantityUpdates`** когда `setQtyToDraft` не потребовал PATCH).
- **Web:** `PartPickerPage` + `PickerSubmitPreviewModal` — выбор режима для строк `quantityUpgrade`, динамический итог в модалке; затем **`POST`** новых SKU и **`PATCH`** только если количество реально меняется; `?picked=` на корзине из **созданных + обновлённых** id (строки только с no-op в `picked` не попадают).
- **Expo:** `wishlist/picker.tsx` — отдельная модалка превью с тем же смыслом и API-вызовами; редирект на **`/wishlist`** с тем же `picked=`.
- **Parity status:** aligned (один доменный превью и один контракт сабмита).

## 3.8.2c Совместимость: отчёт, подбор, панель деталей корзины

- **API:** `GET /api/vehicles/[id]/part-compatibility-report?nodeId=&partMasterId=` → `PartCompatibilityReportWire`; fitment-reports, part-masters, evidence — см. [mototwin_recent_implementation_notes_ru.md](../mototwin_recent_implementation_notes_ru.md).
- **Shared:** `@mototwin/types` (`partMasterId` на SKU/рекомендациях, `WishlistItemSkuInfo.partMasterId`), `@mototwin/domain` (`picker-fitment-labels`, `part-compatibility-report-labels`, `buildWishlistDetailCompatibilitySummary`), `@mototwin/api-client` (`getPartCompatibilityReport`, `getPartMaster`, community endpoints).
- **Подбор (picker):** web `PickerFitmentReportLink` / Expo `picker-fitment-report-link.tsx` — на карточках рекомендаций, в альтернативах и в поиске SKU ссылка **«Отчёт о совместимости →»**; кнопки **«Сбросить выбор»** и **«Добавить свою деталь»**. Маршруты отчёта: web `/parts/fitment-report`, Expo `/wishlist/fitment-report`. Community: web `/parts/community`, Expo `/wishlist/community`.
- **Корзина — панель деталей:** web `WishlistItemCompatibilityBlock` в `PartsCartPage`; Expo `WishlistItemCompatibilityBlock` в нижнем листе `wishlist/index.tsx` — блок **«Совместимость»** (сводка + переход в полный отчёт при `nodeId` + `partMasterId`).
- **Parity status:** aligned по API и смыслу навигации; отличия layout (web: правая колонка; Expo: bottom sheet). Полная страница отчёта и форма «своя деталь» на Expo — stack-экраны, на web — full-page в том же layout.
...
## 3.9 Node picker (выбор листа в модалке)

- **Web:** **`NodePickerModal`** (+ **`NodePickerPopover`**, **`AddNodeSheet`** для multi).
- **Expo:** **`MobileNodePickerModal`**.
- **Shared:** группировка и подписи пути — **`@mototwin/domain`** (`groupNodePickerOptionsByTopLevel`, `nodeAncestorPathLabelRu`, …).
- **Parity status:** одинаковый сценарий списка/поиска/топ-узлов и согласованные подписи при корректной сборке `options`.
- **Spec:** [node-picker-reuse.md(../node-picker-reuse.md); обязательное использование при новых сценариях — [coding-rules.md(../coding-rules.md) §14.
...
3. **Resolved in UI stubs recovery (2026-06)**
- Garage cards on web and Expo use real `attentionSummary.items` from `GET /api/garage`; quick-action labels match destinations (ТО → form, Расход → expenses).
- Web notification settings in Profile; inbox at `/notifications` supports snooze/seen, web push registration, and **primary action button** by `actionLabel`/`actionUrl` (parity with Expo).
- Fitment report community votes wired to `POST /api/fitment/reports/[id]/votes`.
- Expo garage-wide expenses at `/expenses`; install location supports text + Yandex Maps deep link.
- Service event attachment toggles hidden until file storage exists; «Повторить событие» uses `?repeatOf=` flow.
- Admin: `/admin/service-rules` + `/admin/service-rules/new`; `/admin/notifications` delivery log; legacy `/admin/fitment/conflicts*` → `/admin/moderation?queue=mixedFitments`.
- Web garage CTA «Добавить мотоцикл» и dashed add-card: исправлен клик (без `<Link><Button>`).
- Web vehicle dashboard: back arrow → `/garage`.
...
- [frontend-expo.md](../frontend-expo.md)
- [node-picker-reuse.md](../node-picker-reuse.md) — общий выбор узла дерева (web/Expo + domain)
