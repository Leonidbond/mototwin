...
| Форма сервисного события (bundle) | `src/app/vehicles/[id]/_components/service-event-form/` (`ServiceEventForm` + `service-events/new` / `…/edit`) | `apps/app/components/vehicle-detail/basic-service-event-bundle-form.tsx` + экран `service-events/new.tsx` |
