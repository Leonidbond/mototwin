# MotoTwin Cursor Skill: Iterative Parts Catalog Builder (v1.1)

> **Superseded by [mototwin_cursor_parts_catalog_skill_v1_2.md](./mototwin_cursor_parts_catalog_skill_v1_2.md).** Use v1.2 for all new catalog work. Schema details: [parts-catalog-schema.md](./parts-catalog-schema.md).
...
## Regional verification policy — EU / RU / US

This section is mandatory for catalog work where parts will be used by MotoTwin users outside the United States.

### Core rule

Do not treat a US-only catalog result as globally valid.

A US catalog is allowed for bootstrap, but the row must preserve its region and must not be silently converted into EU/RU compatibility.

### Region roles

```text
EU = target source for European/Russian motorcycles when available
RU = regional dealer / availability / purchasing validation, rarely full EPC source
US = useful public bootstrap source, not automatically valid for EU/RU
GLOBAL = only when the official manufacturer source clearly presents global applicability
```

### Approval logic

```text
OFFICIAL_EPC with exact model/year/VIN or model code match
→ may become MANUAL_APPROVED after review

EU official/dealer public catalog with exact model/year/model-code match
→ may become MANUAL_APPROVED after review

US official/dealer public catalog only
→ keep market=US, source_region=US, review_status=NEEDS_REVIEW for EU/RU use

US + EU public sources match by part number and model/year
→ confidence=HIGH, review_status may become MANUAL_APPROVED after human review

RU dealer/source only without exploded diagram or model-code evidence
→ use as availability/purchase reference, not fitment source of truth

Reference-only source match
→ may increase reviewer confidence, but never approves by itself
```
...
If Cursor cannot determine region, set:
...
### Region mismatch rule

If the target model in MotoTwin is EU/RU and the only confirmed source is US, Cursor must add a review note:

```text
Region mismatch: confirmed only in US catalog. Needs EU/RU verification before MANUAL_APPROVED.
```

---

## Brand source strategy
...
```text
1. BMW AOS / EPC — OFFICIAL_EPC, EU/VIN-specific when legally accessible
2. BMW official public / dealer catalog for target region
3. bike-parts-bmw.com — OFFICIAL_DEALER_PUBLIC_CATALOG, EU bootstrap/verification
4. shopbmwmotorcycles.com — OFFICIAL_DEALER_PUBLIC_CATALOG / US bootstrap
5. BMW Motorrad Russia / official RU dealers — regional purchasing/availability validation
6. ETK mirrors / forums — REFERENCE_ONLY
```

BMW-specific rule:

```text
For EU/RU MotoTwin records, prefer AOS/EPC or EU public dealer catalog.
US catalog rows are acceptable as bootstrap but must remain market=US until cross-checked.
```
...
```text
1. KTM Spare Parts Finder — OFFICIAL_PUBLIC_CATALOG
2. Regional KTM official/dealer parts catalog
3. Authorized dealer pages
4. Community/forum/marketplace data — REFERENCE_ONLY only
```

KTM-specific rule:

```text
Prefer model + year + VIN/engine/frame category evidence where available.
```
...
```text
1. Ducati Genuine Part Catalogue — OFFICIAL_PUBLIC_CATALOG
2. Official Ducati dealer/public parts catalogs
3. EU public fiche/dealer catalog
4. Reference mirrors/forums — REFERENCE_ONLY
```

### Yamaha / Honda / Kawasaki / Suzuki / Triumph

Preferred order:

```text
1. Official manufacturer parts catalog / EPC / spare parts finder
2. Official regional dealer public catalogs
3. Authorized dealer catalog pages
4. Public fiche sites connected to official dealer networks
5. Reference mirrors/forums/marketplaces — REFERENCE_ONLY
```

For these brands, Cursor must create `source-map.md` first and explicitly classify each source before collecting parts.

---

## Cross-source verification workflow

For each target node, Cursor must use this sequence:

```text
1. Search primary official / EU source.
2. If found, save exact source evidence.
3. Search US public catalog only as bootstrap or cross-check.
4. Search RU/regional dealer sources only for regional validation, availability or purchase links.
5. Search reference-only sources only if official sources are incomplete or conflicting.
6. Normalize part number.
7. Compare source part numbers.
8. Detect supersession / replacement notes.
9. Set review_status and confidence according to evidence.
10. Write conflicts to review-notes.md.
```
...
```text
EVIDENCE_A:
  OFFICIAL_EPC or manufacturer official catalog, exact model/year/VIN/model-code match

EVIDENCE_B:
  official dealer public catalog, exact model/year/model-code match

EVIDENCE_C:
  authorized dealer page, exact model/year or part-number evidence

EVIDENCE_D:
  reference-only mirror, forum, marketplace, or community mention
```
...
```text
HIGH:
  Evidence A, or Evidence B from target region plus no conflicts,
  or US+EU official/dealer public sources match exactly.

MEDIUM:
  One official/dealer source exists but region mismatch or model-code uncertainty remains.

LOW:
  Reference-only source, unclear region, unclear model code, conflicting part numbers, or missing diagram evidence.
```

### Conflict handling

If two official or dealer-public sources disagree, Cursor must not choose one silently.

Write both rows or one row with conflict note, and set:
...
Every model batch `source-map.md` must include this structure:

```md
# Source map — <Brand> <Model> / <Generation> / <Years>

## Target region
- Primary target region: EU / RU / US / GLOBAL
- MotoTwin support level:
- Notes:
...
## Source decisions
- Which source is primary for this batch:
- Which source is used for cross-check:
- Which sources are excluded and why:
```

---

## Additional CSV fields for regional verification

If the existing schema allows extension, add these columns after `source_region`:

```csv
source_model_code,source_year,verification_region,evidence_level,region_match_status,supersession_status,verified_against_source_url
```

Allowed values:

```text
verification_region: EU | RU | US | GLOBAL | UNKNOWN
...
If changing CSV schema is too disruptive, store these details in `raw_notes` and `review-notes.md` until the importer is updated.

---

## Human review gates for EU/RU usage

For MotoTwin EU/RU users, rows must stay `NEEDS_REVIEW` unless at least one is true:

```text
- confirmed in official EPC / AOS / manufacturer catalog for target model/year;
- confirmed in EU public dealer catalog with exact model/year/model-code match;
- confirmed by two official/dealer-public sources across EU/US with matching part number and no conflict;
- manually reviewed by the MotoTwin catalog owner.
```

Safety-critical nodes remain review-gated even if a source looks strong.
