...
- Тестовое ТС: **KTM 690 Enduro R / 2019-current** (любой `Vehicle` пользователя `demo@mototwin.local` после `prisma db seed`). Узел проверки — **TIRES.FRONT** (узел дерева без бренда). Конкретные `vehicleId`, `nodeId`, `partMasterId` берутся из `/api/garage`, `/api/nodes` и `/api/parts/recommended-skus` — после унификации иерархии моделей хардкоженных IDs больше нет.
- Иерархия мото на тестовом ТС после рефакта: `MotorcycleBrand "KTM" → MotorcycleModelFamily "690 Enduro R" → MotorcycleVariant "690 Enduro R" → MotorcycleGeneration "690 Enduro R / 2019-current"` (см. `docs/models/mototwin_ktm_model_technical_master_cursor.md`).
...
После `prisma migrate reset && prisma db seed` ID-шники меняются — резолвим их на лету:

```bash
BASE=http://127.0.0.1:3000

VID=$(curl -sS "$BASE/api/garage" | jq -r '.vehicles[0].id')
NODE=$(curl -sS "$BASE/api/nodes" | jq -r '.nodes[] | select(.code=="TIRES.FRONT") | .id')
PM=$(curl -sS "$BASE/api/parts/recommended-skus?vehicleId=$VID&nodeId=$NODE" | jq -r '.recommendations[] | select(.partMasterId) | .partMasterId' | head -n1)
