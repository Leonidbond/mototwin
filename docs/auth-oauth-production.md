...
| `OAuthCallback` / `PKCE code_verifier cookie was missing` (Apple) | POST callback без cross-site cookies | деплой с `buildAuthCookies()` в authjs.ts; HTTPS prod |
| `invalid_client` (Apple) | истёк JWT secret или неверный Key/Team/Service ID | перегенерировать `AUTH_APPLE_CLIENT_SECRET` |
