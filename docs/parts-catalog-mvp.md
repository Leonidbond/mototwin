...
**OEM staging pipeline (реализовано, v1.2):** CSV → `PartCatalogApplication` → promote → `PartSku` / `PartFitment`. Контракт, import и admin: [`catalog/parts-catalog-schema.md`](./catalog/parts-catalog-schema.md). Этот документ описывает **production SKU-слой** и community `PartMaster`; staging — отдельный ingest-слой с полным provenance.
...
## 17. Админка: управление и удаление каталога

Операционное управление PartMaster / SKU / fitment — через web admin (не прямые правки БД).

| Действие | UI | API / роли |
| --- | --- | --- |
| Список и фильтры | `/admin/catalog` | `GET /api/admin/parts` — любой admin |
| Создание | кнопка на `/admin/catalog` | `POST /api/admin/parts` — `SUPER_ADMIN`, `CATALOG_MANAGER` |
| Редактирование | `/admin/catalog/[id]` | `PATCH /api/admin/parts/[id]` — + `MODERATOR` |
| Merge дубликатов | вкладка «Дубликаты» | `POST /api/admin/parts/[id]/merge` |
| **Массовое удаление** | чекбоксы + панель на `/admin/catalog` | `POST /api/admin/parts/bulk-delete` — `{ ids, reason }`, max 50 |
| **Одиночное удаление** | «Удалить из каталога» на вкладке «Информация» | тот же bulk-delete с одним `id` |
| Staging CSV | `/admin/catalog/staging` | approve/reject → promote |

**RBAC:** destructive delete — только `SUPER_ADMIN` и `CATALOG_MANAGER` (`canDeleteCatalogParts()`).

**Что удаляется при hard delete:** PartMaster, связанные SKU (номера, узлы, fitment-правила, офферы), community fitment (отчёты, голоса, confidence, aliases). Позиции wishlist остаются, `skuId` обнуляется.

**Ops / сброс тестовых данных:** `npx tsx scripts/parts/purge-catalog-data.ts [--dry-run]` — полная очистка каталога в БД без re-import.

Подробнее: [admin-panel-readme.md](./admin-panel-readme.md), [api-backend.md](./api-backend.md) §3.10, [catalog skill v1.2](./catalog/mototwin_cursor_parts_catalog_skill_v1_2.md).
