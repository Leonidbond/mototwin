...
const svgDir = path.join(root, "images", "top-node-icons");
const pngDir = path.join(svgDir, "preview");
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="${STROKE}" stroke-width="${STROKE_WIDTH}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" role="img">
  <title>${title}</title>
${body}
</svg>
`;
...
// 1. Двигатель - стилизованный блок цилиндра с рёбрами охлаждения и крепёжной базой.
const engine = svg(
  "Двигатель",
...
// 2. Тормоза - тормозной диск с вентиляционными прорезями и суппортом справа.
const brakes = svg(
  "Тормоза",
...
// 3. Шины - покрышка, диск, спицы и редкие метки протектора.
function tiresBody() {
  const lines = [
...
const tires = svg("Шины", tiresBody());

// 4. Цепь/звёзды - силуэт звёздочки (зубчатое кольцо) и цепочка роликов справа.
function chainBody() {
  const cx = 8;
  const cy = 12;
  const rOuter = 5;
...
const chain = svg("Цепь и звёзды", chainBody());

// 5. Смазка - капля масла с бликом, символизирует моторное масло/фильтр.
const lubrication = svg(
  "Смазка",
...
// 6. Подвеска - шток амортизатора с винтовой пружиной и верхней/нижней проушинами.
const suspension = svg(
  "Подвеска",
...
async function main() {
  for (const [name, content] of Object.entries(icons)) {
    fs.writeFileSync(path.join(svgDir, `${name}.svg`), content);
  }

  const PREVIEW_COLOR = "#e2e7ef";
  for (const [name, content] of Object.entries(icons)) {
    const coloured = content.replace(/stroke="currentColor"/, `stroke="${PREVIEW_COLOR}"`);
    await sharp(Buffer.from(coloured), { density: 512 })
      .resize({ width: 96, height: 96, fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } })
      .png()
      .toFile(path.join(pngDir, `${name}.png`));
  }

  const order = ["lubrication", "engine", "brakes", "tires", "chain", "suspension"];
  const previewSizes = [24, 32, 48, 96];
  const padding = 24;
  const cellHeader = 28;

  const cellWidth = previewSizes.reduce((sum, s) => sum + s + padding, padding);
  const cellHeight = Math.max(...previewSizes) + padding + cellHeader;
  const cols = 2;
  ...
  for (let i = 0; i < order.length; i++) {
    const name = order[i];
    const coloured = icons[name].replace(/stroke="currentColor"/, `stroke="${PREVIEW_COLOR}"`);
    const col = i % cols;
    const row = Math.floor(i / cols);

    let x = col * cellWidth + padding;
    const baseY = row * cellHeight + cellHeader;

    for (const size of previewSizes) {
      const tile = await sharp(Buffer.from(coloured), { density: 1024 })
        .resize({ width: size, height: size, fit: "contain", background: { r: 0, g: 0, b: 0, alpha: 0 } })
        .png()
        .toBuffer();
      const y = baseY + (Math.max(...previewSizes) - size);
      overlays.push({ input: tile, top: y, left: x });
      x += size + padding;
    }
  }
  ...
  await sharp({
    create: {
      ...
      height: gridHeight,
      channels: 4,
      background: { r: 13, g: 17, b: 24, alpha: 1 },
    },
  })
    .composite(overlays)
    .png()
    .toFile(path.join(pngDir, "grid.png"));

  process.stdout.write(`generated ${Object.keys(icons).length} svg icons + previews\n`);
}
